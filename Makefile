...
# 🐳 Docker Commands
.PHONY: build up down logs clean install run-dev stop-backend backend-status run-backend run-frontend run-backend-with-env nuke-all nuke-db fresh-start
...
stop-backend:
	...
	@pkill -f "go run cmd/server/main.go" 2>/dev/null && echo "✅ Go backend stopped" || echo "ℹ️  No Go backend running"
	@pkill -f "./server" 2>/dev/null && echo "✅ Binary backend stopped" || echo "ℹ️  No binary backend running"
...
backend-status:
	...
	@echo ""
	@echo "🔍 Go processes:"
	@pgrep -fl "go run cmd/server/main.go" || echo "   No Go backend running"
	@echo ""
	@echo "🔍 Binary processes:"
	@pgrep -fl "./server" || echo "   No binary backend running"
	@echo ""
...
db-import:
	...
	docker exec -i blueprint-postgres-dev psql -U postgres -d blueprint < init.sql
...
nuke-db:
	...
	docker volume rm blueprint_postgres-dev-data blueprint_redis-dev-data blueprint_timescale-data 2>/dev/null || true
...
help:
	...
	@echo "  make run-backend    - Run backend server locally (auto-restart)"
