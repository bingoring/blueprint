...
func main() {
	...
	// ⚖️ Blueprint Court 분쟁 해결 시스템 초기화
	juryService := services.NewJuryService()
	disputeService := services.NewDisputeService(sseService, juryService)
	...
	arbitrationHandler := handlers.NewArbitrationHandler(arbitrationService) // 🏛️ 기존 분쟁 해결 핸들러 추가
	disputeHandler := handlers.NewDisputeHandler(disputeService) // ⚖️ Blueprint Court 분쟁 핸들러 추가
	...
	// 🏛️ 공개 분쟁 해결 정보
	api.GET("/arbitration/stats", arbitrationHandler.GetArbitrationStats)           // 기존 분쟁 해결 통계 (공개)
